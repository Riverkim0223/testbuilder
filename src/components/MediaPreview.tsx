...
import {
  getChallengePreviewImageUrl,
  getYouTubeMatchUrl,
  getYouTubeThumbnailUrl,
  resolveChallengeYoutubeUrl,
} from '@/utils/youtubeMatches';
...
export default function MediaPreview({
  ...
  const resolvedYoutubeUrl = resolveChallengeYoutubeUrl(challengeId, youtubeUrl);

  const videoSrc = previewVideoUrl ?? (challengeId ? `/previews/${challengeId}.mp4` : undefined);
  const imageSrc =
    (challengeId ? getChallengePreviewImageUrl(challengeId) : undefined) ??
    previewImageUrl ??
